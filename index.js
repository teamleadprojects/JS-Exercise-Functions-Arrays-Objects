...
function appleIndex(fruit) {
  const index = fruit.findIndex(fruit => fruit === "apple");
  return index;
  /* code here */
}
...
function getCarInfoById(inventory, id) {
  let selectedCar;
  for (let i = 0; i < inventory.length; i++) {
    const currentCarInv = inventory[i];
    if (currentCarInv.id === id) {
      selectedCar = currentCarInv;
    }
  }
  return `This is a ${selectedCar.car_make} ${selectedCar.car_model}`;
...
function getGermanCars(inventory) {
  let germCar = [];
  for (let i = 0; i < inventory.length; i++) {
    const carNum = inventory[i];
    if (
      carNum.car_make === "Audi" ||
      carNum.car_make === "Mercedes-Benz" ||
      carNum.car_make === "Volkswagen" ||
      carNum.car_make === "BMW"
    ) {
      germCar.push(carNum);
    }
